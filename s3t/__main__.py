...
def parse_args():
    ...
    parser.add_argument("-d", "--download", type=str, dest="Download a file from a bucket")
    parser.add_argument("-u", "--upload", type=str, dest="Upload a file to a bucket")
    parser.add_argument("-c", "--copy", type=str, dest="Copy a file from one bucket to another")
    parser.add_argument("-r", "--remove", type=str, dest="Remove a file from a bucket")
    parser.add_argument("-t", "--target", type=str, dest="Target bucket (for use with --copy)")
    parser.add_argument("-k", "--key", type=str, dest="Name to save a file as (for --upload, --copy, --download)")
    parser.add_argument("-l", "--list", type=str, nargs="?", const="*", dest="List files on a bucket")
    parser.add_argument("-a", "--all", action="store_true", default=False, dest="List all files (no pagination)")
    parser.add_argument("-s", "--settings", action="store_true", default=False, dest="Define default settings")
    parser.add_argument("-v", "--version", action="version", version=VERSION)
    parser.add_argument("-b", "--bucket", default=conf["default_bucket"], dest="Use specified bucket")
    parser.add_argument("--creds", type=str, nargs="?", const="?", dest="Run the tool with credentials")
